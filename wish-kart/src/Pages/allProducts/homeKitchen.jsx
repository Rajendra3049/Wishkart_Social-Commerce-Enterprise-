...
const Men = () => {
  ...
  useEffect(() => {
    let Data = data.filter((el) => el.category === "Home and Kitchen");
    setHomeData(Data);
  }, [data.length]);
  ...
  useEffect(() => {
    if (filtCred !== "") {
      if (
        ...
        let RatingData = homeData.filter(
          (el) =>
            (filtCred.ratingTop ? el.rating >= 4.5 : null) ||
    ...
    return (
      <Box mt={["100px", "100px", "200px"]}>
        ...
        <Flex>
          <Hide below="md">
            <Box width={"20%"}>
              <Heading color={"#f43397"} marginLeft="7%">
                Filter
              </Heading>
              <Box>
                <Accordion>
